...
stage=1
lm_dir=data/local/lm/unigram-mincount
...
# 移除aishell2使用kaldi训练语言模型，使用ngram训练语言模型
# arpa LM
mkdir -p $lm_dir

# 如果报ngram-count错误，可能是ngram没有安装或者没有设置环境变量, 可以
# 先设置环境变量或者指定ngram-count的路径
ngram-count -text ../corpus/words.txt -order 1 -unk -map-unk "<UNK>" -interpolate -lm $lm_dir/lm_unpruned.gz || exit 1;

# G compilation, check LG composition
if [ $stage -le 5 ]; then
  utils/format_lm.sh data/lang $lm_dir/lm_unpruned.gz \
